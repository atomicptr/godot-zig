...
var mbind_visual_shader_node_is_constructor: ?fn () ?*c_api.godot_object = null;

pub const VisualShaderNodeIs = struct {
    const Self = @This();
    ...
    pub fn init() !*Self {
        if (mbind_visual_shader_node_is_constructor == null) {
            mbind_visual_shader_node_is_constructor = try api.createConstructor("VisualShaderNodeIs");
        }
        return api.createObject(Self, mbind_visual_shader_node_is_constructor.?);
    }
    ...
    pub fn getFunction(self: *const Self) !i32 {
        if (mbind_get_function == null) {
            mbind_get_function = try api.createMethod("VisualShaderNodeIs", "get_function");
        }
    ...
    pub fn setFunction(self: *const Self, arg_func: i32) !void {
        if (mbind_set_function == null) {
            mbind_set_function = try api.createMethod("VisualShaderNodeIs", "set_function");
        }
